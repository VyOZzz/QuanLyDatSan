...
public interface PriceRuleRepository extends JpaRepository<PriceRules, Long>{
    ...
    /**
     * Tìm tất cả quy tắc giá của một venues
     */
    List<PriceRules> findByVenues(Venues venues);

    /**
     * Tìm quy tắc giá áp dụng cho một thời điểm cụ thể tại một venues.
     * Lưu ý: Xử lý trường hợp khung giờ qua đêm (ví dụ: 22:00 - 05:00).
     */
    @Query("SELECT pr FROM PriceRules pr WHERE pr.venues.id = :venueId AND pr.active = true AND " +
