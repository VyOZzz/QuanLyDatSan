...
public class BookingController {
    ...
    @PreAuthorize("hasRole('OWNER')")
    @Operation(summary = "Lấy danh sách booking chờ xác nhận của một venue cụ thể",
               description = "Trả về các booking có status PAYMENT_UPLOADED của một venue. Owner phải là chủ sở hữu của venue đó.")
    public ResponseEntity<ApiResponse<List<BookingResponse>>> getPendingBookingsByVenue(@PathVariable Long venueId) {
        List<BookingResponse> bookings = bookingService.getPendingBookingsByVenueId(venueId);
        return ResponseEntity.ok(ApiResponse.ok(bookings, "Lấy danh sách booking chờ xác nhận thành công."));
    }
